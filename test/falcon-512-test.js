...
contract("Falcon", accounts => {
  ...
  describe("Signature type", async() => {
    ...
    it("has to have the first nibble of the signature equal to 3 or 5 if signature type is 0", async() => {
      let signature = Buffer.alloc(42);
      signature.write('0A', 0, 1, 'hex');
      let pubKey = Buffer.from('0A', 'hex');
      let signatureType = 0;
    ...
    it("has to have proper signature length in the public key if the signature type is 0", async() => {
      let signature = Buffer.alloc(42);
